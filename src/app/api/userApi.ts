...
const transformUserProfile = (profile: UserProfileResponse): User => {
  ...
  const shouldIncludeDistributorInfo =
    profile.is_distributor || profile.referral_code;
  ...
  return {
    ...
    distributorInfo: shouldIncludeDistributorInfo
      ? {
          ...
          totalReferrals:
            (profile.left_leg_count || 0) + (profile.right_leg_count || 0),
...
export const userApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getUserProfile: builder.query<User, void>({
      queryFn: async () => {
        ...
        try {
          ...
          console.log(
            "🔄 [USER PROFILE API] Transformed user:",
            transformedUser,
          );

          // Store profile picture in localStorage if it exists and has changed
          if (data.profile_picture) {
            const currentStoredPicture = localStorage.getItem(
              "ev_nexus_profile_picture",
            );
            if (currentStoredPicture !== data.profile_picture) {
              setProfilePicture(data.profile_picture);
              console.log(
                "✅ [USER PROFILE API] Profile picture updated in localStorage",
              );
            }
          } else {
            ...
            if (currentStoredPicture) {
              ...
              console.log(
                "✅ [USER PROFILE API] Profile picture cleared from localStorage",
              );
            }
          }
        ...
        try {
          ...
          if (data.profile_picture) {
            ...
            if (currentStoredPicture !== data.profile_picture) {
              setProfilePicture(data.profile_picture);
              console.log(
                "✅ [USER PROFILE RAW API] Profile picture updated in localStorage",
              );
            }
          } else {
            // Clear profile picture if it's not in the response
            ...
            if (currentStoredPicture) {
              setProfilePicture(null);
              console.log(
                "✅ [USER PROFILE RAW API] Profile picture cleared from localStorage",
              );
            }
          }
    ...
    submitKYC: builder.mutation<KYCSubmissionResponse, KYCSubmissionRequest>({
      queryFn: async (body) => {
        console.log(
          "🔵 [KYC API] ========== Starting KYC submission ==========",
        );
        ...
        const tokenPreview =
          accessToken.length > 20
            ...
            : "***";
        console.log(
          "🔵 [KYC API] Access token found in localStorage:",
          tokenPreview,
        );
        ...
        try {
          ...
          console.log("📤 [KYC API] Request Body:", {
            ...
            pan_document: body.pan_document
              ? `File: ${body.pan_document.name} (${body.pan_document.size} bytes, type: ${body.pan_document.type})`
              : "null",
            aadhaar_front: body.aadhaar_front
              ? `File: ${body.aadhaar_front.name} (${body.aadhaar_front.size} bytes, type: ${body.aadhaar_front.type})`
              : "null",
            aadhaar_back: body.aadhaar_back
              ? `File: ${body.aadhaar_back.name} (${body.aadhaar_back.size} bytes, type: ${body.aadhaar_back.type})`
              : "null",
            bank_passbook: body.bank_passbook
              ? `File: ${body.bank_passbook.name} (${body.bank_passbook.size} bytes, type: ${body.bank_passbook.type})`
              : "null",
          });
          ...
          console.log("🔵 [KYC API] Request headers:", {
            ...
            "Content-Type":
              "multipart/form-data (with boundary - set by browser)",
          });
          ...
          console.log(
            "📥 [KYC API] Response status:",
            response.status,
            response.statusText,
          );
          console.log(
            "📥 [KYC API] Response headers:",
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [KYC API] Access token expired, attempting to refresh...",
            );
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                ...
                console.log(
                  "📤 [KYC API] Retry FormData entries:",
                  Array.from(formData.entries()).map(([key, value]) => ({
                    key,
                    value:
                      value instanceof File
          ...
          console.log(
            "✅ [KYC API] Success! Response data:",
            JSON.stringify(data, null, 2),
          );
          console.log(
            "✅ [KYC API] ========== KYC submission completed ==========",
          );
          return { data };
        } catch (error) {
    ...
    updateProfile: builder.mutation<
      UserProfileResponse,
      Partial<UserProfileResponse> & { profile_picture?: File }
    >({
      queryFn: async (body) => {
        console.log(
          "🔵 [PROFILE UPDATE API] ========== Starting Profile Update ==========",
        );
        // Type guard helper
        ...
        console.log("🔵 [PROFILE UPDATE API] Request body type check:", {
          ...
          profilePictureValue:
            isProfilePictureFile && profilePicture
        ...
        try {
          ...
          if (hasProfilePicture && profilePicture) {
            ...
            console.log(
              "📤 [PROFILE UPDATE API] Added profile_picture file to FormData:",
              {
            ...
            console.log(
              "📤 [PROFILE UPDATE API] Using multipart/form-data (profile_picture included)",
            );
            ...
            console.log(
              "📤 [PROFILE UPDATE API] FormData entries:",
              formDataEntries,
            );
            ...
            if (response.status === 401) {
              console.log(
                "🟡 [PROFILE UPDATE API] Access token expired, attempting to refresh...",
              );
              const refreshData = await refreshAccessToken();
              ...
              if (refreshData) {
                ...
                if (newAccessToken) {
                  console.log(
                    "🔄 [PROFILE UPDATE API] Retrying request with new token...",
                  );
                  // Recreate FormData (can't reuse after being sent)
                  const retryFormData = new FormData();
                  if (body.first_name)
                    retryFormData.append("first_name", body.first_name);
                  if (body.last_name)
                    retryFormData.append("last_name", body.last_name);
                  ...
                  if (body.date_of_birth)
                    retryFormData.append("date_of_birth", body.date_of_birth);
                  if (body.address_line1)
                    retryFormData.append("address_line1", body.address_line1);
                  if (body.address_line2)
                    retryFormData.append("address_line2", body.address_line2);
                  if (body.city) retryFormData.append("city", body.city);
                  if (body.state) retryFormData.append("state", body.state);
                  if (body.pincode)
                    retryFormData.append("pincode", body.pincode);
                  if (body.country)
                    retryFormData.append("country", body.country);
            ...
            const data: UserProfileResponse = await response.json();
            console.log(
              "✅ [PROFILE UPDATE API] Profile updated successfully with multipart/form-data",
            );
            ...
            if (data.profile_picture) {
              ...
              if (currentStoredPicture !== data.profile_picture) {
                setProfilePicture(data.profile_picture);
                console.log(
                  "✅ [PROFILE UPDATE API] Profile picture updated in localStorage",
                );
              }
            } else {
              // Clear profile picture if it's not in the response
              ...
              if (currentStoredPicture) {
                setProfilePicture(null);
                console.log(
                  "✅ [PROFILE UPDATE API] Profile picture cleared from localStorage",
                );
              }
            }

            console.log(
              "✅ [PROFILE UPDATE API] ========== Profile Update Completed ==========",
            );
            return { data };
          } else {
            // If no profile_picture, use application/json
            console.log(
              "📤 [PROFILE UPDATE API] Using application/json (no profile_picture)",
            );
            // Remove profile_picture from body if it exists but isn't a File
            const { profile_picture, ...jsonBody } = body;
            console.log(
              "📤 [PROFILE UPDATE API] Request body:",
            ...
            const data: UserProfileResponse = await response.json();
            console.log(
              "✅ [PROFILE UPDATE API] Profile updated successfully with application/json",
            );
    ...
    submitNominee: builder.mutation<
      NomineeSubmissionResponse,
      NomineeSubmissionRequest
    >({
      queryFn: async (body) => {
        const isUpdate = !!body.nomineeId;
        const method = isUpdate ? "PUT" : "POST";
        console.log(
          `🔵 [NOMINEE API] ========== Starting Nominee ${isUpdate ? "update" : "submission"} ==========`,
        );
        ...
        if (!accessToken) {
          console.error(
            "❌ [NOMINEE API] No access token found in localStorage",
          );
          return {
            error: {
              status: 401,
        ...
        const tokenPreview =
          accessToken.length > 20
            ...
            : "***";
        console.log(
          "🔵 [NOMINEE API] Access token found in localStorage:",
          tokenPreview,
        );
        ...
        try {
          // Create FormData for multipart/form-data
          console.log(
            `🔵 [NOMINEE API] Creating FormData from request body (${method})...`,
          );
          const formData = new FormData();
          ...
          console.log(
            `📤 [NOMINEE API] FormData entries (${method}):`,
            formDataEntries,
          );
          console.log(
            `📤 [NOMINEE API] FormData entries (stringified) (${method}):`,
          ...
          console.log(`📤 [NOMINEE API] ${method} Request Body Summary:`, {
            ...
            fields: {
              ...
              id_proof_document: body.id_proof_document
                ? "Included"
                : "Not included",
            },
          ...
          console.log(
            `📥 [NOMINEE API] Response status (${method}):`,
            response.status,
            response.statusText,
          );
          console.log(
            `📥 [NOMINEE API] Response headers (${method}):`,
            Object.fromEntries(response.headers.entries()),
          );
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({
                message: `Failed to ${isUpdate ? "update" : "submit"} nominee details`,
              }));
            console.error(
              `❌ [NOMINEE API] Error response (${method}):`,
              error,
            );
            return {
              error: {
          ...
          console.log(
            `✅ [NOMINEE API] Success! Response data (${method}):`,
            JSON.stringify(data, null, 2),
          );
          console.log(
            `✅ [NOMINEE API] ========== Nominee ${isUpdate ? "update" : "submission"} completed ==========`,
          );
          return { data };
        } catch (error) {
    ...
    getNomineeDetails: builder.query<NomineeDetails | null, void>({
      queryFn: async () => {
        console.log(
          "🔵 [NOMINEE GET API] ========== Starting Nominee fetch ==========",
        );
        ...
        try {
          ...
          console.log(
            "📥 [NOMINEE GET API] Response status:",
            response.status,
            response.statusText,
          );
          console.log(
            "📥 [NOMINEE GET API] Response headers:",
            Object.fromEntries(response.headers.entries()),
          );
          ...
          if (!response.ok) {
            ...
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to fetch nominee details" }));
          ...
          console.log(
            "✅ [NOMINEE GET API] Success! Response data:",
          ...
          console.log(
            "✅ [NOMINEE GET API] Extracted nominee data:",
            JSON.stringify(nomineeData, null, 2),
          );
          console.log(
            "✅ [NOMINEE GET API] ========== Nominee fetch completed ==========",
          );
    ...
    submitNomineeKYC: builder.mutation<NomineeKYCSubmissionResponse, void>({
      queryFn: async () => {
        console.log(
          "🔵 [NOMINEE KYC API] ========== Starting Nominee KYC Submission ==========",
        );
        ...
        if (!accessToken) {
          console.error(
            "❌ [NOMINEE KYC API] No access token found in localStorage",
          );
          return {
            error: {
              status: 401,
        ...
        try {
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [NOMINEE KYC API] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [NOMINEE KYC API] Retrying request with new token...",
                );
                response = await fetch(apiUrl, {
                  method: "POST",
                  headers: {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to submit nominee KYC" }));
          ...
          console.log(
            "✅ [NOMINEE KYC API] ========== Nominee KYC Submission Completed ==========",
          );
          return { data };
        } catch (error) {
    ...
    >({
      queryFn: async (params = {}) => {
        ...
        try {
          ...
          if (params.is_distributor !== undefined)
            queryParams.append(
              "is_distributor",
              params.is_distributor.toString(),
            );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - getNormalUsers] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - getNormalUsers] Retrying request with new token...",
                );
    ...
    getUserById: builder.query<UserProfileResponse, number>({
      queryFn: async (userId) => {
        ...
        try {
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - getUserById] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - getUserById] Retrying request with new token...",
                );
    ...
    >({
      queryFn: async ({ userId, data }) => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - updateUserById] Request URL:",
            `${API_BASE_URL}users/${userId}/`,
          );
          console.log(
            "📤 [USER API - updateUserById] Request Body:",
            requestBody,
          );
          console.log(
            "📤 [USER API - updateUserById] Request Body (Parsed):",
            JSON.stringify(data, null, 2),
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - updateUserById] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - updateUserById] Retrying request with new token...",
                );
                response = await fetch(`${API_BASE_URL}users/${userId}/`, {
                  method: "PUT",
                  headers: {
          ...
          if (!response.ok) {
            ...
            console.error(
              "❌ [USER API - updateUserById] Error Response:",
              error,
            );
            return {
              error: {
          ...
          console.log(
            "✅ [USER API - updateUserById] Response Status:",
            response.status,
          );
          console.log(
            "✅ [USER API - updateUserById] Response Data:",
            JSON.stringify(responseData, null, 2),
          );
      ...
      queryFn: async ({ userId, total_earned }) => {
        ...
        try {
          ...
          if (response.status === 401) {
            ...
            if (response.status === 401) {
              const error = await response
                ...
                .catch(() => ({ message: "Authentication failed" }));
              return {
                error: { status: 401, data: error } as FetchBaseQueryError,
              };
            }
          }
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to update total earnings" }));
            return {
              error: {
                status: response.status,
                data: error,
    ...
    >({
      queryFn: async (userId) => {
        ...
        try {
          console.log(
            "📤 [USER API - deleteUserById] Request URL:",
            `${API_BASE_URL}users/${userId}/`,
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - deleteUserById] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - deleteUserById] Retrying request with new token...",
                );
                response = await fetch(`${API_BASE_URL}users/${userId}/`, {
                  method: "DELETE",
                  headers: {
          ...
          if (!response.ok) {
            ...
            console.error(
              "❌ [USER API - deleteUserById] Error Response:",
              error,
            );
            return {
              error: {
          ...
          console.log(
            "✅ [USER API - deleteUserById] Response Status:",
            response.status,
          );
          console.log(
            "✅ [USER API - deleteUserById] Response Data:",
            JSON.stringify(responseData, null, 2),
          );
    ...
    >({
      queryFn: async (params) => {
        ...
        try {
          ...
          if (params.submitted_date_from)
            queryParams.append(
              "submitted_date_from",
              params.submitted_date_from,
            );
          if (params.submitted_date_to)
            queryParams.append("submitted_date_to", params.submitted_date_to);
          if (params.reviewed_date_from)
            queryParams.append("reviewed_date_from", params.reviewed_date_from);
          if (params.reviewed_date_to)
            queryParams.append("reviewed_date_to", params.reviewed_date_to);
          if (params.user_id)
            queryParams.append("user_id", params.user_id.toString());
          if (params.user_role)
            queryParams.append("user_role", params.user_role);
          if (params.ordering) queryParams.append("ordering", params.ordering);
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - getKYCList] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - getKYCList] Retrying request with new token...",
                );
                response = await fetch(url, {
                  method: "GET",
                  headers: {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to fetch KYC list" }));
            return {
              error: {
                status: response.status,
                data: error,
          ...
          const data = await response.json();
          console.log(
            "📥 [USER API - getKYCList] Response:",
            JSON.stringify(data, null, 2),
          );
          console.log("📥 [USER API - getKYCList] Response Count:", data.count);
          console.log(
            "📥 [USER API - getKYCList] Results Count:",
    ...
    updateKYCStatus: builder.mutation<
      { success: boolean; message: string },
      { kycId: number; status: "approved" | "rejected"; reason?: string }
    >({
      queryFn: async ({ kycId, status, reason }) => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - updateKYCStatus] Request URL:",
            `${API_BASE_URL}users/kyc/${kycId}/update-status/`,
          );
          console.log(
            "📤 [USER API - updateKYCStatus] Request Body:",
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - updateKYCStatus] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - updateKYCStatus] Retrying request with new token...",
                );
                response = await fetch(
                  `${API_BASE_URL}users/kyc/${kycId}/update-status/`,
                  {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: `Failed to ${status} KYC` }));
            console.error(
              "❌ [USER API - updateKYCStatus] Error Response:",
              error,
            );
            return {
              error: {
          ...
          const data = await response.json();
          console.log(
            "✅ [USER API - updateKYCStatus] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
    ...
    approveNomineeKYC: builder.mutation<
      { success: boolean; message: string },
      { nomineeId: number }
    >({
      queryFn: async ({ nomineeId }) => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - approveNomineeKYC] Request URL:",
            `${API_BASE_URL}users/nominee/${nomineeId}/approve/`,
          );
          console.log(
            "📤 [USER API - approveNomineeKYC] Request Body:",
            JSON.stringify(requestBody, null, 2),
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - approveNomineeKYC] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - approveNomineeKYC] Retrying request with new token...",
                );
                response = await fetch(
                  `${API_BASE_URL}users/nominee/${nomineeId}/approve/`,
                  {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to approve nominee KYC" }));
            console.error(
              "❌ [USER API - approveNomineeKYC] Error Response:",
              error,
            );
            return {
              error: {
          ...
          const data = await response.json();
          console.log(
            "✅ [USER API - approveNomineeKYC] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
    ...
    rejectNomineeKYC: builder.mutation<
      { success: boolean; message: string },
      { nomineeId: number; reason: string }
    >({
      queryFn: async ({ nomineeId, reason }) => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - rejectNomineeKYC] Request URL:",
            `${API_BASE_URL}users/nominee/${nomineeId}/reject/`,
          );
          console.log(
            "📤 [USER API - rejectNomineeKYC] Request Body:",
            JSON.stringify(requestBody, null, 2),
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - rejectNomineeKYC] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - rejectNomineeKYC] Retrying request with new token...",
                );
                response = await fetch(
                  `${API_BASE_URL}users/nominee/${nomineeId}/reject/`,
                  {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to reject nominee KYC" }));
            console.error(
              "❌ [USER API - rejectNomineeKYC] Error Response:",
              error,
            );
            return {
              error: {
          ...
          const data = await response.json();
          console.log(
            "✅ [USER API - rejectNomineeKYC] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
    ...
    >({
      queryFn: async () => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - getDistributorApplications] Request URL:",
            url,
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - getDistributorApplications] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - getDistributorApplications] Retrying request with new token...",
                );
                response = await fetch(url, {
                  method: "GET",
                  headers: {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({
                message: "Failed to fetch distributor applications",
              }));
            return {
              error: {
                status: response.status,
          ...
          const data = await response.json();
          console.log(
            "📥 [USER API - getDistributorApplications] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
          console.error(
            "❌ [USER API - getDistributorApplications] Error:",
            error,
          );
          return {
            error: {
    ...
    >({
      queryFn: async (userId) => {
        ...
        try {
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - getUserDocuments] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - getUserDocuments] Retrying request with new token...",
                );
                response = await fetch(url, {
                  method: "GET",
                  headers: {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({ message: "Failed to fetch user documents" }));
            console.error(
              "❌ [USER API - getUserDocuments] Error Response:",
              error,
            );
            return {
              error: {
          ...
          const data = await response.json();
          console.log(
            "✅ [USER API - getUserDocuments] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
    ...
    >({
      queryFn: async ({ applicationId, status, reason }) => {
        ...
        try {
          ...
          console.log(
            "📤 [USER API - updateDistributorApplicationStatus] Request URL:",
            url,
          );
          console.log(
            "📤 [USER API - updateDistributorApplicationStatus] Request Body:",
            JSON.stringify(requestBody, null, 2),
          );
          ...
          if (response.status === 401) {
            console.log(
              "🟡 [USER API - updateDistributorApplicationStatus] Access token expired, attempting to refresh...",
            );
            const refreshData = await refreshAccessToken();
            ...
            if (refreshData) {
              ...
              if (newAccessToken) {
                console.log(
                  "🔄 [USER API - updateDistributorApplicationStatus] Retrying request with new token...",
                );
                response = await fetch(url, {
                  method: "POST",
                  headers: {
          ...
          if (!response.ok) {
            const error = await response
              .json()
              .catch(() => ({
                message: `Failed to ${status} distributor application`,
              }));
            console.error(
              "❌ [USER API - updateDistributorApplicationStatus] Error Response:",
              error,
            );
            return {
              error: {
          ...
          const data = await response.json();
          console.log(
            "✅ [USER API - updateDistributorApplicationStatus] Response:",
            JSON.stringify(data, null, 2),
          );
          return { data };
        } catch (error) {
          console.error(
            "❌ [USER API - updateDistributorApplicationStatus] Error:",
            error,
          );
          return {
            error: {
